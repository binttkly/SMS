...
public class UserServiceImpl implements UserService {
    ...
    public ResultVo showList(String name,int pageNum) {
        ...
        List<User1> list = userDao.showList(name);
        PageInfo<User1> pageInfo=new PageInfo<User1>(list);
     ...
     */
    @Override
    public ResultVo addUser(User1 user) {
        User1 selectUser = userDao.getUserByName(user.getName());
        if(selectUser==null){
            //设置默认密码是123
    ...
    @Override
    public ResultVo updateUser(User1 user) {
